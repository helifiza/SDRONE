...
class Prediction(Base):
    ...
    drone_type_scire: Mapped[dict | None] = mapped_column(JSON, nullable= True)
    ...
    updated_at: Mapped[datetime] = mapped_column(DateTime, server_default= "CURRENT_TIMESTAMP", onupdate= datetime.utcnow,)
...
class stft_image(Base):
    ...
    pre_mavic_ent: Mapped[float] = mapped_column(Float, nullable= True)
    drone_bin: Mapped[bool] = mapped_column(Boolean, nullable=True)
    __table_args__ = (
        UniqueConstraint("predict_id", "segment_idx", name= "uq_predict_segment"),
    )
